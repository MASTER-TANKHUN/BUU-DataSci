...
function horizontalBars(el,data,{labelKey='label',valueKey='value',format=v=>v.toFixed(2),color='#2bb79a',max=null,tooltipFormat=null}={}){
  clear(el);
  const isMobile=mobile(el),{w}=size(el),rowH=isMobile?42:46,h=Math.max(280,data.length*rowH+30);
  const m=isMobile?{l:92,r:24,t:10,b:20}:{l:170,r:44,t:10,b:20},iw=w-m.l-m.r;
  const svg=svgEl('svg',{viewBox:`0 0 ${w} ${h}`});svg.style.height=h+'px';el.style.minHeight=h+'px';el.appendChild(svg);
  const mx=max||Math.max(...data.map(d=>d[valueKey]),1);
  data.forEach((d,i)=>{
    const y=m.t+i*rowH,bh=isMobile?18:20,bw=iw*d[valueKey]/mx;
    const label=svgEl('text',{x:m.l-8,y:y+13,'text-anchor':'end',class:'axis'});label.textContent=d[labelKey];svg.appendChild(label);
    svg.appendChild(svgEl('rect',{x:m.l,y,width:iw,height:bh,rx:2,fill:'#edf1f3'}));
    const r=svgEl('rect',{x:m.l,y,width:bw,height:bh,rx:2,fill:typeof color==='function'?color(d,i):color,class:'bar'});svg.appendChild(r);
    if(!isMobile){const val=svgEl('text',{x:Math.min(w-m.r-2,m.l+bw+8),y:y+14,class:'axis'});val.textContent=format(d[valueKey]);svg.appendChild(val);}
    r.addEventListener('mousemove',e=>{const rect=el.getBoundingClientRect();const content=tooltipFormat?tooltipFormat(d):`<b>${d[labelKey]}</b><br>${format(d[valueKey])}`;tip(el,content,e.clientX-rect.left,e.clientY-rect.top)});
    r.addEventListener('mouseleave',()=>hideTip(el));
  });
}
